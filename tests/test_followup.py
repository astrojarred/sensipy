...
def test_get_row_found(sample_lookup_df):
    """Test get_row when row is found."""
    row = followup.get_row(
        lookup_df=sample_lookup_df,
        event_id=1,
        irf_site="north",
        irf_zenith=20,
...
def test_get_row_not_found(sample_lookup_df):
...
def test_get_row_multiple_matches(sample_lookup_df):
    """Test get_row when multiple rows match (should use first)."""
    # Add duplicate row
    duplicate_row = sample_lookup_df.iloc[0].copy()
    duplicate_df = pd.concat([sample_lookup_df, duplicate_row.to_frame().T], ignore_index=True)
...
def test_get_row_empty_filters(sample_lookup_df):
...
def test_get_row_invalid_column(sample_lookup_df):
...
def test_extrapolate_obs_time_valid(sample_lookup_df):
...
def test_extrapolate_obs_time_below_minimum(sample_lookup_df):
...
def test_extrapolate_obs_time_above_maximum(sample_lookup_df, capsys):
...
def test_extrapolate_obs_time_other_info(sample_lookup_df):
...
def test_extrapolate_obs_time_no_matching_data(sample_lookup_df):
...
def test_get_sensitivity_conflicting_inputs(sample_lookup_df):
...
def test_get_exposure_with_extrapolation_df(sample_lookup_df, mock_csv_path):
...
def test_get_exposure_without_extrapolation_df(mock_csv_path, sample_lookup_df):
...
def test_get_exposure_invalid_delay_unit(mock_csv_path, sample_lookup_df):
...
def test_get_exposure_invalid_energy_units(mock_csv_path, sample_lookup_df):
...
def test_get_exposure_invalid_radius_unit(mock_csv_path, sample_lookup_df):
...
def test_get_exposure_custom_parameters(mock_csv_path, sample_lookup_df):
...
def test_get_exposure_no_matching_data(sample_lookup_df):
...
def test_get_exposure_without_irf_site(sample_lookup_df):
...
def test_extrapolate_obs_time_custom_column_names(sample_lookup_df):
    """Test extrapolate_obs_time with custom column names."""
    # Rename columns
    df = sample_lookup_df.rename(columns={
        "obs_delay": "delay_col",
        "obs_time": "time_col"
    })
...
def test_extrapolate_obs_time_missing_column(sample_lookup_df):
...
def test_get_exposure_custom_column_names(sample_lookup_df):
