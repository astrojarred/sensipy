...
# Assume source and sens are already set up (see examples above)
# with min_energy = 30 * u.GeV and max_energy = 10 * u.TeV
...
# Assume source and sens are already set up with min_energy = 30 * u.GeV
result = source.observe(
    sensitivity=sens,
    start_time=30 * u.min,
    min_energy=min_energy,
