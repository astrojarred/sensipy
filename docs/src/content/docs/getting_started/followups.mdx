...
Two columns are required: `obs_delay` and `obs_time`, which are the observation delay and the $5\sigma$ observation time for the event, respectively. The columns names do not have to match exactly.

| Column          | Type    | Required | Description                                          |
| --------------- | ------- | -------- | ---------------------------------------------------- |
| `obs_delay`     | float   | Yes      | Observation delay time (seconds)                    |
| `obs_time`      | float   | Yes      | $5\sigma$ observation time (seconds)                  |
...
- The table may not have data for this event—try a different one
